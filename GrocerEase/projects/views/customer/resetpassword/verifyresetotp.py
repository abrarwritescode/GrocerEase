...
def verifycustomerresetotp(request, email):
    if request.method == 'POST':
        ...
        if entered_otp == customer.otp:
            ...
            return render(request, 'customer/verify_reset_otp.html', {'error_message': error_message, 'email': email})

    return render(request, 'customer/verify_reset_otp.html', {'email': email})